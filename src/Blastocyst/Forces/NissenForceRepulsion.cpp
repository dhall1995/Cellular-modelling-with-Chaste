...
NissenForceRepulsion<ELEMENT_DIM,SPACE_DIM>::NissenForceRepulsion()
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
c_vector<double, SPACE_DIM> NissenForceRepulsion<ELEMENT_DIM,SPACE_DIM>::CalculateForceBetweenNodes(unsigned nodeAGlobalIndex,
                                                                                            ...
                                                                                            AbstractCellPopulation<ELEMENT_DIM,SPACE_DIM>& rCellPopulation)
{
    ...
    potential_gradient = -exp(-d)*unit_vector_from_A_to_B;
    c_vector<double, SPACE_DIM> force;

    if (this->mUseCutOffLength)
    {
        if (d/2.0 >= this->GetCutOffLength())  //remember chaste distances given in DIAMETERS so we define CutOffLength as 2.5 Diameters
        {
            return force;
        }
    }
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double NissenForceRepulsion<ELEMENT_DIM,SPACE_DIM>::GetGrowthDuration()
{
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void NissenForceRepulsion<ELEMENT_DIM,SPACE_DIM>::SetGrowthDuration(double GrowthDuration)
{
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void NissenForceRepulsion<ELEMENT_DIM,SPACE_DIM>::OutputForceParameters(out_stream& rParamsFile)
{
...
template class NissenForceRepulsion<1,1>;
template class NissenForceRepulsion<1,2>;
template class NissenForceRepulsion<2,2>;
template class NissenForceRepulsion<1,3>;
template class NissenForceRepulsion<2,3>;
template class NissenForceRepulsion<3,3>;
